...
use N8G\Grass\Components\Html\Input;
use N8G\Utils\Log;
...
class InputTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testInstantiation($content, $id, $elements, $attributes)
	{
		//No arguments
		$element = new Input();
		$this->assertInstanceOf('N8G\Grass\Components\Html\Input', $element);

		//Just ID
		$element = new Input($id);
		$this->assertInstanceOf('N8G\Grass\Components\Html\Input', $element);

		//ID and attributes
		$element = new Input($id, $attributes);
		$this->assertInstanceOf('N8G\Grass\Components\Html\Input', $element);
	}
	...
	{
		$element = new Input($content, $id, $elements, $attributes);
	}
	...
	{
		$element = new Input('test', array('class', 'test'));
	...
	public function testValidateAttributes()
	{
		...
		$element->setAttributes(array('type' => 'button', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'checkbox', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'color', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'date', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'datetime', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'datetime-local', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'email', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'file', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'hidden', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'image', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'month', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'number', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'password', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'radio', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'range', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'reset', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'search', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'submit', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'tel', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'text', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'time', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'url', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');

		$element->setAttributes(array('type' => 'week', 'name' => 'test'));
		$this->invokeMethod($element, 'validateAttributes');
	...
	public function toStringProvider()
	{
		...
		//Add fixtures
		$data[0]['fixture'] = 'Input element\n\r    Attributes: type=text, name=test';
		$data[1]['fixture'] = 'Input element\n\r    Attributes: type=text, name=test';
		$data[2]['fixture'] = 'Input element\n\r    ID: test\n\r    Attributes: type=text, name=test';
		$data[3]['fixture'] = 'Input element\n\r    Attributes: type=text, name=test';
		$data[4]['fixture'] = 'Input element\n\r    Attributes: type=text, name=test, class=test';
		$data[5]['fixture'] = 'Input element\n\r    Attributes: type=text, name=test';
		$data[6]['fixture'] = 'Input element\n\r    ID: 1\n\r    Attributes: type=text, name=test';
	...
	public function toHtmlProvider()
	{
		...
		//Add fixtures
		$data[0]['fixture'] = '<input type="text" name="test" />';
		$data[1]['fixture'] = '<input type="text" name="test" />';
		$data[2]['fixture'] = '<input id="test" type="text" name="test" />';
		$data[3]['fixture'] = '<input type="text" name="test" />';
		$data[4]['fixture'] = '<input type="text" name="test" class="test" />';
		$data[5]['fixture'] = '<input type="text" name="test" />';
		$data[6]['fixture'] = '<input id="1" type="text" name="test" />';
	...
	private function genericDataProvider()
	{
		return array(
			array(
				...
				'id'			=>	null,
				'elements'		=>	array(),
				'attributes'	=>	array('type' => 'text', 'name' => 'test')
			),
			array(
				'content'		=>	'Test',
				'id'			=>	null,
			...
			array(
				...
				'id'			=>	null,
				'elements'		=>	array(),
				'attributes'	=>	array('type' => 'text', 'name' => 'test', 'class' => 'test')
			),
			array(
				'content'		=>	1,
				'id'			=>	null,
